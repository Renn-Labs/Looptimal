#!/usr/bin/env bash
# run-this-loop.sh — engine-agnostic loop runner generated by Looptimal.
#
...
# ---- the four atoms (filled by Looptimal from loop-spec.yaml) ----------------
...
# no-op is exactly the failure Looptimal exists to prevent, so surface it loudly at the start (don't fail: an
